...
def addresut():
    def Subresult():
        ...
        if cmb_quest.get()=="CT:-1" or cmb_quest.get()=="CT:-2":
            ...
            try:
                ...
                mycursor.execute(strr, (roll,name,cmb_quest.get(),s,cd, T))
                con.commit()
    ...
    cd = Label(addroot, text="CD:-", font=("times", 20, "italic bold"), bg="lavender", relief=GROOVE, borderwidth=5,
                 width=15)
    cd.place(x=10, y=250)
    ...
    cdentry = Entry(addroot, font=("times", 20, "italic bold"), relief=GROOVE, bd=5, textvariable=cdval)
    cdentry.place(x=280, y=250)
...
"RollNo", "Name", "Exam", "CD",  "TotalMarks"),
...
StudentTable.heading("CD", text="CD")
...
StudentTable.column("CD",width=90)
