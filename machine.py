...
def addresut():
    def Subresult():
        ...
        if cmb_quest.get()=="CT:-1" or cmb_quest.get()=="CT:-2":
            ...
            try:
                ...
                mycursor.execute(strr, (roll,name,cmb_quest.get(),s,ML, T))
                con.commit()
    ...
    ML = Label(addroot, text="ML:-", font=("times", 20, "italic bold"), bg="lavender", relief=GROOVE, borderwidth=5,
                 width=15)
    ML.place(x=10, y=250)
    ...
    MLentry = Entry(addroot, font=("times", 20, "italic bold"), relief=GROOVE, bd=5, textvariable=MLval)
    MLentry.place(x=280, y=250)
...
style.configure("Treeview", font=("times", 15, " italic bold"), foreground="white", background="cornflowerblu")
Scroll_x = Scrollbar(data, orient=HORIZONTAL)
Scroll_y = Scrollbar(data, orient=VERTICAL)
StudentTable = ttk.Treeview(data, columns=(
"RollNo", "Name", "Exam", "ML",  "TotalMarks"),
...
StudentTable.heading("ML", text="ML")
...
StudentTable.column("ML",width=90)
